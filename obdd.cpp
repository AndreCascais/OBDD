...
*
* DESCRIPTION :
*       VFS assignment for obdd's with the operations given in the class.
*
*       
*
* AUTHORS :   Andre Cascais up201103896
*             Andre Diegues up201206858
*
*H************************************************************************/
...
void print_node(Node* node) {
    ...
    printf("My var is %c, my label is %d my idx is %d\n", node->var, node->label, node->idx);
}
...
void run() {
    ...
    char const* help_msg = "help                       - shows this message\n"
        ...
        "exists VAR OBDD1 OBDD2     - apply(or, OBDD1[0/VAR], OBDD1[1/VAR]) into OBDD2\n"
        "forall VAR OBDD1 OBDD2     - apply(and, OBDD1[0/VAR], OBDD1[1/VAR]) into OBDD2\n"
        "exit                       - exits removing all the obdds\n";
    ...
    while(1) {
        ...
        if (strcmp(token, "help") == 0) {
            ...
            printf("File %s created with sucess\n", token);
        }
            ...
            free(new_vars);
            obdd_map[id4] = o4;
            printf("Exists successful, %s created\n", token);
        }
            ...
            free(new_vars);
            obdd_map[id4] = o4;
            printf("Forall successful, %s created\n", token);
        }

        else if (!strcmp(token, "exit")) {
            for (std::map<string, OBDD*>::iterator itr = obdd_map.begin(); itr != obdd_map.end(); itr++) {
                delete itr->second;
            }
            return;
        }
